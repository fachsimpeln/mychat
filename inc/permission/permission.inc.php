<?php
     ...
     class PermissionHandler
     {
          ...
          public function GetChatPermission($usrID, $receiverUsername)
          {
               ...
               $stmt = $this->pdo->prepare("SELECT `fr_id` FROM `mc_friendlist` WHERE ((`usr_id1`=:senderid OR `usr_id2`=:senderid) AND (`usr_id1`= (SELECT `usr_id` FROM `mc_users` WHERE `usr_username`=:receiverUsername) OR `usr_id2`=(SELECT `usr_id` FROM `mc_users` WHERE `usr_username`=:receiverUsername)))");
